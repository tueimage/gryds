    ...
    class TestBSplineCudaTransformation(TestCase):
        ...
        def test_translation_bspline_2d(self):
            ...
            np.testing.assert_equal(
                new_grid.grid[0, -1, 0], np.array(1.9, DTYPE))
        ...
        def test_translation_bspline_5d(self):
            ...
            # Translation by 100% will mean that the i-axis will now run from 1 to 1.9
            np.testing.assert_equal(
                new_grid.grid[0, 0, 0, 0, 0], np.array(1, DTYPE))
            np.testing.assert_almost_equal(
                new_grid.grid[0, -1, 0, 0, 0], np.array(1.6666667, DTYPE))
        ...
        def test_bspline_2d(self):
            ...
            np.testing.assert_almost_equal(
                new_grid.grid[0, 0, 0], np.array(0.1, DTYPE))
